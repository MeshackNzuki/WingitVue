...
namespace App\Http\Controllers\Inventory\Quotation;

use App\Enums\QuotationStatus;
use App\Models\Inventory\Product;
use App\Models\Inventory\Customer;
use App\Models\Inventory\Quotation;
use App\Models\Inventory\QuotationDetails;
...
class QuotationController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('quotations.create', [
            ...
            // 'statuses' => QuotationStatus::cases() // Uncomment if needed
        ]);
    }
            ...
            foreach (Cart::instance('quotation')->content() as $cart_item) {
                ...
                if ($request->status == 1) {
                    Product::where('id', $cart_item->id)
                        ->decrement('quantity', $cart_item->qty);
                }
            }
    ...
    {
        $quotation->update([
            'status' => 2 // Mark as deleted or cancelled
        ]);
    ...
    public function update(Request $request, $uuid)
    {
        ...
        $quotation->load(['customer', 'quotationDetails']);
        $quotation->status = 1; // Completed status
        
        foreach ($quotation->quotationDetails as $product) {
