<template>
    <div class="h-full ml-14 mb-10 md:ml-64">
      ...
      <div class="relative flex flex-wrap mt-10 rounded-lg">
        <div class="w-full mb-12 px-4">
          <Table
            :headers="['FROM', 'TO', 'PRICE', 'DEPART TIME', 'ARRIVAL TIME', 'ALL SEATS', 'BOOKED SEATS', 'REMAINING SEATS']"
            title="Listed Flights (Available in the Booking Window)"
            v-model:query="query"
          >
            <template v-slot:search>
              <input
                class="h-6 hidden md:block bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-60 pl-10"
            ...
            <template v-slot:content>
              <tr v-for="(flight, index) in filteredFlights" :key="index">
                ...
                <td class="border-t-0 px-2 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 items-center">
                  {{ flight.initial_seats - flight.available_seats }}
                </td>
                <td class="border-t-0 px-2 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 items-center">
  ...
  const filteredFlights = computed(() => {
    ...
    return flights.value.filter((flight) => {
      return (
        flight.origin_airport.name.toLowerCase().includes(query.value.toLowerCase()) ||
        flight.destination_airport.name.toLowerCase().includes(query.value.toLowerCase())
      );
    });
  });
