<template>
    <div class="bg-slate-100 dark:bg-sky-950 min-h-screen rounded-md">
        <div class="flex flex-wrap justify-around my-6 py-12 w-full gap-4">
            <StatCard label="Listed flights" description="verified"
                :quantity="mainStore.tourOpData?.listed_flights_count || '0'" icon="pi pi-th-large" />
            <StatCard label="Previous flights" description="Approved"
                :quantity="mainStore.tourOpData?.previous_flights_count || '0'" icon="pi pi-list" />
            <StatCard label="Guests" description="Approved" :quantity="mainStore.airOpData?.pilots_count || '0'"
                icon="pi pi-user" />
            <StatCard label="Revenue" description="32% Increase"
                :quantity="mainStore.tourOpData?.total_revenue_count || '0'" icon="pi pi-sync" />
        </div>
        <div class="flex mt-4">
                ...
                <div class="w-full flex flex-col justify-center items-center h-[30vh]">
                    <div class="flex flex-col gap-4 mb-4">
                        <div class="flex flex-row gap-4">
                            <SmallStatCard :quantity="mainStore.tourOpData?.total_revenue_count
                                " />
                            <SmallStatCard />
                        </div>
                        <div class="flex flex-row gap-4">
...
const fetchDashData = async () => {
    const response = await axios.get("/dashdata-tourop");
    mainStore.setDashDataTourOp(response.data);
};
fetchDashData();
</script>
